...
#define STREAM_WRITE_INT8(s,d) s.write(reinterpret_cast<char*>(&d),sizeof(uint8_t))
#define STREAM_READ_INT8(s,d) if(!s.eof()){s.read(reinterpret_cast<char*>(&d),sizeof(uint8_t));}else{throw exception("Input stream ended to early");}
